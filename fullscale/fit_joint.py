...
def parse_args(argv=None):
    ...
    p.add_argument("--edm-tol", type=float, default=1e-3,
                   help="THE convergence criterion: rabbit's own EDM, "
                        "0.5 g^T H^-1 g, the NLL still to be gained WITH the "
                        "parameter correlations. EDM < 1e-3 is a worst-"
                        "direction displacement of 0.045 sigma.")
...
def main(argv=None):
    ...
    for nm in pois:
        ...
        print(f"      {nm:>12s} grad {gj_[i_]:+12.5g}  "
              f"FULL step {step[i_]:+9.4f} sigma")
