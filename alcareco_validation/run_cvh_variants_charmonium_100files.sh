...
#
# Output layout:
#   /ceph/submit/data/user/d/david_w/ZMass/cvh/260506_variants_charmonium_100files/
...
OUTBASE=/ceph/submit/data/user/d/david_w/ZMass/cvh/260506_variants_charmonium_100files
mkdir -p $OUTBASE
...
prepare_one() {
    ...
    mkdir -p $outdir
    rm -f $outdir/*.root
    cfg=/tmp/run_${label}_${variant}_100files.py
    inject_variant $src $attr $cfg $vtx $pt
    echo "${label}_${variant} $cfg $outdir $outfile"
}

KS_RUNNER=/work/submit/david_w/ZMass/CMSSW_10_6_26/src/Analysis/HitAnalyzer/test/runGlobalCorRecKsFromCharmonium100files.py
LM_RUNNER=/work/submit/david_w/ZMass/CMSSW_10_6_26/src/Analysis/HitAnalyzer/test/runGlobalCorRecLambdaFromCharmonium100files.py
JP_RUNNER=/work/submit/david_w/ZMass/CMSSW_10_6_26/src/Analysis/HitAnalyzer/test/runCvhJpsiCandidateDriven100files.py
