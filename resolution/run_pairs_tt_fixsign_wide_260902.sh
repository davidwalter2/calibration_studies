...
run_part() { local f=$1; local tag; tag=$(basename "$(dirname "$f")"); local out="$PARTS/part_$tag.npz"
  [ -s "$out" ] && { echo "[skip] $tag"; return 0; }
  if python3 "$SCRIPT" --pairs-tt --ioni-sign neg --files "$f" --ntasks 1 --pairs-cache "$out" > "$PARTS/part_$tag.log" 2>&1; then echo "[done] $tag $(date +%H:%M)"; else echo "[FAIL] $tag"; rm -f "$out"; fi; }
export -f run_part; export PARTS SCRIPT
ls -d "$PROD"/task_*/globalcor_0.root | sort | awk 'NR>14' | xargs -P 146 -I{} bash -c 'run_part {}'
