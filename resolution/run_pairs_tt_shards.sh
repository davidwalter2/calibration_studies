...
run_part() { local f=$1; local tag; tag=$(basename "$(dirname "$f")"); local out="$PARTS/part_$tag.npz"
  ...
  if python3 cf_mass_likelihood.py --pairs-tt --files "$f" --ntasks 1 --pairs-cache "$out" > "$PARTS/part_$tag.log" 2>&1
...
ls -d "$PROD"/task_*/globalcor_0.root | sort | xargs -P "$NPAR" -I{} bash -c 'run_part {}'
