...
# EVERY STREAM OF EVERY USABLE TASK. `ls task_*/globalcor_0.root` takes only a
# quarter of the candidates from a numberOfThreads=4 production, and picks up
...
run_shard() {
  ...
  [ -s "$list" ] || return 0
  # One shard = one explicit file list, handed to --files directly -- NOT a
  # directory of symlinks named task_NNNN/globalcor_0.root, which a
  # multi-stream production breaks twice over: the fake tasks carry no
  # `.complete`, and a task's four streams would have to be renamed apart.
