#!/bin/bash
# Progress of the 8.5M DY MiniAOD CVH two-track (Z -> mumu) production.
#
...
NAMES=dymc8p5M_a0,dymc8p5M_a1,dymc8p5M_a2
...
if (( SLOW )); then
  ...
  done | sort | uniq -c | awk '{printf "streams/task : %d task(s) with %s file(s)%s\n", $1, $2, ($3>0 ? " -- " $3 " EMPTY" : "")}'
