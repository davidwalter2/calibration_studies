...
NAME=dymc_v2
# Tasks of CHUNKLIST this production actually runs: indices 0..NTASKS_USED-1.
# status/resume read it so a SUBSET production is not reported against 380.
NTASKS_USED=${NTASKS_USED:-40}
...
# hep.wisc.edu: SIGILL (rc 132) in InitRootHandlers::ThreadTracker inside the
# OVERLAY's own pluginFWCoreServicesPlugins.so -- 5 of the first 5 failures of
# the 260917 DY submission, on 5 DIFFERENT Wisconsin nodes, i.e. a SITE CPU
# generation, not black-hole nodes. The area's libraries are built
# x86-64-v3 (vfmadd/shlx/lzcnt/andn are present) while the cvmfs base release
# ships scram_x86-64-v2, so any pre-Haswell execute node kills the job the
# moment a payload .so is loaded. Since 2026-09-16 the overlay also carries
# FWCore/Services, which is loaded during service construction -- that is why
# this surfaces now and as an instant crash rather than mid-fit.
REQUIREMENTS=${REQUIREMENTS:-'(regexp("swan.hcc.unl.edu", Machine, "i") =!= true) && (regexp("cmsplt02", Machine, "i") =!= true) && (regexp("node-0011.hepgrid.uerj.br", Machine, "i") =!= true) && (regexp("cism.ucl.ac.be", Machine, "i") =!= true) && (regexp("physik.rwth-aachen.de", Machine, "i") =!= true) && (regexp("jinr.ru", Machine, "i") =!= true) && (regexp("s1wn17\.pi\.infn\.it", Machine, "i") =!= true) && (regexp("compute-12n-5\.ultralight\.org", Machine, "i") =!= true) && (regexp("compute-21-23\.ultralight\.org", Machine, "i") =!= true) && (regexp("node38-4\.wn\.iihe\.ac\.be", Machine, "i") =!= true) && (regexp("hep.wisc.edu", Machine, "i") =!= true) && (regexp("compute-6-34\.ultralight\.org", Machine, "i") =!= true) && (regexp("compute-6-6\.ultralight\.org", Machine, "i") =!= true) && (regexp("compute-22-12\.ultralight\.org", Machine, "i") =!= true) && (regexp("compute-12n-23\.ultralight\.org", Machine, "i") =!= true) && (regexp("compute-21-22\.ultralight\.org", Machine, "i") =!= true) && (regexp("t2bat0310\.cmsaf\.mit\.edu", Machine, "i") =!= true)'}
...
EXTRA="doRes=True exportCfExponents=True exportStepRecords=False \
 ...
 exportVtxResidual=True bsConstraint=True exportBsResidual=True \
