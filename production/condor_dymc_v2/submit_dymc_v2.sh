#!/bin/bash
# Submit the DY MiniAOD CVH two-track re-production to HTCondor (CMS global
# pool).  The condor twin of production/submit_dymc8p5M.sh: same chunk list,
# same task index -> same event range, same `.complete` sentinel, same
# PROVENANCE.txt written next to the output.
#
...
#     is the exact failure config_dymc_v2.sh exists to prevent.
...
#
# usage: ./submit_dymc_v2.sh [--dry-run] [--only "0 1 2"] [--idxfile FILE]
...
APPEND=(
  -append "JOBSH        = $HERE/job_dymc_v2.sh"
...
if (( DRY )); then
  printf 'condor_submit '; printf '%q ' "${APPEND[@]}" "$HERE/dymc_v2.sub"; echo
  echo "--- first indices ---"; head -5 "$HERE/.submit_idx.txt"; exit 0
fi
condor_submit "${APPEND[@]}" "$HERE/dymc_v2.sub"
