#!/bin/bash
# DIAGNOSTIC twin of job_dymc_v2.sh.  Identical payload, identical cmsRun
# command line; the ONLY differences are instrumentation:
...
#     took the signal and leaves only the paused ones).
# Never used by the production itself.
...
# ---- memory sampler (diagnostic only) -------------------------------------
# Condor samples MemoryUsage every ~5 min, which is far too coarse to tell a
# steady leak from a single huge allocation. 15 s resolution, and the last
# processed record so growth can be tied to an event number.
memtrace() {
  local pid=$1
  echo "# t_s VmRSS_kB VmHWM_kB VmSize_kB nthreads last_record"
  while [[ -d /proc/$pid ]]; do
    local rss hwm vsz thr ev
    ...
    ev=$(grep -o 'Begin processing the [0-9]*' local.log 2>/dev/null | tail -1 | grep -o '[0-9]*$')
    echo "$(( $(date +%s) - T0 )) ${rss:-0} ${hwm:-0} ${vsz:-0} ${thr:-0} ${ev:-0}"
    sleep 15
  done
}
...
CMSPID=$!
memtrace "$CMSPID" > memtrace.txt 2>/dev/null &
MEMPID=$!
wait "$CMSPID"
rc=$?
...
echo ">>> memtrace peak: $(awk 'NR>1 && $3>m {m=$3; l=$0} END{print l}' memtrace.txt)"

# ---- the diagnostic stage-out: ALWAYS, before any exit --------------------
DESTDIR="${OUTROOT%/}/$TASK"
DEST="${OUTHOST%/}/$DESTDIR"
XHOST=${OUTHOST#root://}; XHOST=${XHOST%/}
timeout 300 xrdfs "$XHOST" mkdir -p "$DESTDIR" >/dev/null 2>&1
for f in local.log memtrace.txt; do
  [[ -s "$f" ]] && timeout 900 xrdcp -f -p -N "$f" "$DEST/$f" >/dev/null 2>&1
done

if [[ $rc -ne 0 ]]; then tail -400 local.log >&2; fail "cmsRun rc=$rc" "$rc"; fi
