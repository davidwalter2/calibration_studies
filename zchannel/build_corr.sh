...
[ -s data/kern_corr_data_multi.npz ] || $P fsr_perleg.py corr --htable $HT \
...
S() { t=$1; shift
  [ -s data/kern_corr_mc_$t.npz ] || $P fsr_perleg.py corr --htable $HT10 \
     --run $RUN -o data/kern_corr_mc_$t.npz \
     --acceptance data/acc_corr_mc_$t.json --rho data/rho_${t}_mc.npz "$@"
  [ -s data/kern_corr_data_$t.npz ] || $P fsr_perleg.py corr --htable $HT10 \
